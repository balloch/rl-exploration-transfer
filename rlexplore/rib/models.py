...
class RelationalActorCritic(nn.Module):
    ...
    def forward(self, x):
        ...
        y_coord = torch.div(c, ncols, rounding_mode='trunc').view(*x.shape[:-2], -1, 1)
        print(x.device)
        print(x_coord.device)
        print(y_coord.device)
        x = x.to('cuda:0')
        x_coord = x_coord.to('cuda:0')
        y_coord = y_coord.to('cuda:0')
